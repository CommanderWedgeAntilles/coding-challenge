...
def temperature_celsius(city):
    #city required in the parameter
    city_temp = weather.get_temperature_by_city(city)
    cels_temp = weather.convert_fahrenheit_to_celsius(city_temp)
    return f"The temperature in {city} is {city_temp} ({cels_temp} Celsius)"
